...
5. Write `.agent-dev/requirement.json` combining data from ALL fetched pages:
   ...
   ```
6. Write `.agent-dev/state.json`: `{ "pipelineId": "pipeline-<timestamp>", "sessionId": "${CLAUDE_SESSION_ID}", "phase": "FETCH", "notionUrl": "<url>", "createdAt": "<ISO>", "updatedAt": "<ISO>" }`
...
4. Decision tree (**VERDICT is primary, confidence is secondary**):
   ...
   - **VERDICT == ESCALATE** OR reviewRevisionCount >= 3:
     → Update state.json: phase → ESCALATED
...
4. Decision tree:
   ...
   - **FIX_REQUIRED + codeReviewCount >= 2** or unresolvable:
     → phase → ESCALATED, present to user
...
3. PR body template:
   ...
   **<title>**
   ...
   ```
4. Update state.json: phase → COMPLETED, prUrl → <url>
5. Report: "Pipeline 完成. PR: <url>"
6. If otherProjects in state.json:
   "还有 <otherProjects> 需要实现。要继续下一个项目吗？"
7. Ask: "清理 .agent-dev/ 文件？"
